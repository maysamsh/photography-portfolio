	...
	function getExifDataMarkup(img) {
		...
		function formatExifValue(tag, value) {
			if (tag === 'DateTimeOriginal') {
				// Format: YYYY:MM:DD HH:MM:SS -> 12 January 2026
		...
		for (var current in exif) {
			...
			if (typeof exif_data !== "undefined") {
				var formatted = formatExifValue(current_data['tag'], exif_data);
				template += '<i class="' + current_data['icon'] + '" aria-hidden="true"></i> ' + formatted + '&nbsp;&nbsp;';
			}
		}
		return template;
	}
