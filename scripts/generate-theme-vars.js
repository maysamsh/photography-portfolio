...
function parseThemeFromConfig(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('custom_theme:')) {
      inTheme = true;
      inColors = false;
      continue;
    }
...
function generateScss(theme) {
  ...
  if (theme.font_family) {
    lines[4] = `$theme-font-family: ${theme.font_family};`;
  }
  if (theme.font_family_fixed) {
    lines[5] = `$theme-font-family-fixed: ${theme.font_family_fixed};`;
  }

  // Color overrides - map config keys to palette keys
  const colorMap = {
    bg: 'bg',
    bg_alt: 'bg-alt',
    fg: 'fg',
    ...
    border: 'border',
    border_bg: 'border-bg',
    border_bg_alt: 'border-bg-alt',
    accent: 'accent1'
  };

  const colors = theme.colors || {};
  if (Object.keys(colors).length > 0) {
    const bgHex = colors.bg || '#242629';
    const entries = [];
    if (colors.bg) {
...
// Add a "custom_theme:" section to _config.yml to customize colors and fonts.
$theme-font-family: null;
$theme-font-family-fixed: null;
$theme-palette-overrides: ();
`;
